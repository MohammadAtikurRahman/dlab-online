...
function AllIntervalDownload() {
  ...
  const downloadCSV = () => {
    const csvData = convertToCSV(intervalData);
    const blob = new Blob([csvData], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", "all-interval-info.csv");
  ...
  return (
    <div>
      <button className="btn btn-danger w-100" onClick={downloadCSV} disabled={loading || error || intervalData.length === 0}>
        {loading ? "Loading..." : "Download All Interval Info"}
      </button>
